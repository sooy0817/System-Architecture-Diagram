...
class MeCabTokenSplitter:
    """MeCab-based tokenizer that yields (token, start, end) tuples.

    Falls back to simple whitespace-ish splitting if MeCab isn't available.
    """
    __slots__ = ("tagger", "available", "mecab_error")
    ...
    def __init__(self):
        try:
            ...
            self.available = True
            self.mecab_error = None
        except Exception:
            self.tagger = None
            self.available = False
            # capture traceback for debugging
            self.mecab_error = traceback.format_exc()
    ...
    def __call__(self, text: str, lower: bool = True) -> Iterator[Tuple[str, int, int]]:
        ...
        if not self.available:
            # Fallback: yield non-space runs
            for m in re.finditer(r"\S+", proc_text):
                yield m.group(), m.start(), m.end()
            return

        node = self.tagger.parseToNode(proc_text)
        pos = 0
        while node:
            surface = node.surface
            if surface:
                # Find the next occurrence from current cursor to get char offsets
                start = proc_text.find(surface, pos)
                if start == -1:
                    start = pos
...
class SchemaTransformer:
    ...
    def __init__(
        ...
        # Determine word splitter: explicit > MeCab (if available) > whitespace fallback
        if word_splitter is not None:
            self.word_splitter = word_splitter
        else:
            try:
                # Prefer MeCab if installed
                mecab_checker = MeCabTokenSplitter()
                if getattr(mecab_checker, "available", False):
                    self.word_splitter = mecab_checker
                else:
